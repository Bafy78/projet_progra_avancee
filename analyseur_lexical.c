...
typejeton get_next_token(const char **input, typejeton prev_jeton)
{
    typejeton token;
    ...
    else if (isalpha(c))
    {
        ...
        else
        {
            ...
            {
                token.lexem = ERREUR;
                token.valeur.erreur = 101; // Code d'erreur pour fonction inconnue
            }
        }
    }
    else
...
typejeton *analyseur(const char *expression)

{
    ...
    do
    {
        token = get_next_token(&input, prev_jeton);
        // printf("%d\n", prev_jeton.lexem);
